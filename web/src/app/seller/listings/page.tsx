...
const STATUS_STYLES: Record<Listing["status"], { label: string; color: string; bg: string }> = {
  ...
  archived: { label: "Archived", color: "text-ink-faded", bg: "bg-bg-soft border-rule" },
};
                ...
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <Link
                      ...
                      className="rounded-lg border border-rule px-3 py-1.5 text-xs text-ink-soft transition hover:bg-bg-soft hover:text-ink"
                    >
                      View
                    </Link>
                    {l.status !== "archived" && (
                      <button
                        ...
                        className={`rounded-md border px-3 py-1.5 text-xs transition ${
                          l.status === "active"
                            ? "border-rule text-ink-faded hover:border-accent/20 hover:text-accent"
                            : "border-accent/30 text-accent hover:bg-accent-pale"
                        }`}
                      >
                    ...
                    {l.status === "archived" && (
                      <button
                        onClick={() => toggleStatus(l)}
                        className="rounded-lg border border-rule px-3 py-1.5 text-xs text-ink-faded transition hover:text-ink"
                      >
                        Reactivate
                      </button>
                    )}
